...
class BP_NetDecoder:
    def __init__(self, H, batch_size):
        ...
        self.llr_placeholder = tf.compat.v1.placeholder(tf.float32, [batch_size, self.v_node_num])
        self.llr_into_bp_net, self.xe_0, self.xe_v2c_pre_iter_assign, self.start_next_iteration, self.dec_out = self.build_network()
        self.llr_assign = self.llr_into_bp_net.assign(tf.transpose(a=self.llr_placeholder))  # transpose the llr matrix to adapt to the matrix operation in BP net decoder.

        init = tf.compat.v1.global_variables_initializer()
        self.sess = tf.compat.v1.Session() # open a session
    ...
    def one_bp_iteration(self, xe_v2c_pre_iter, H_sumC_to_V, H_sumV_to_C, xe_0):
        xe_tanh = tf.tanh(tf.cast(tf.truediv(xe_v2c_pre_iter, [2.0]), dtype=tf.float64))
        xe_tanh = tf.cast(xe_tanh, dtype=tf.float32)
    ...
    def build_network(self): # build the network for one BP iteration
        ...
        dec_out = tf.transpose(a=tf.math.floordiv(1-tf.cast(tf.sign(bp_out_llr), dtype=tf.int32), 2))
