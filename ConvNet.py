...
class ConvNet:
    ...
    def build_network(self, built_for_training=False):  # build a network similar with SRCNN, a fully-convolutional netowrk for channel noise estimation
        ...
        for layer in range(self.net_config.total_layers):  # construct layers
            ...
            if layer == self.net_config.total_layers - 1:
                layer_output[layer] = tf.nn.conv2d(input=x_input, filters=self.conv_filter[layer], strides=[1, 1, 1, 1], padding='SAME') + self.bias[layer]
            else:
                # Activation Function
                layer_output[layer] = tf.nn.relu(tf.nn.conv2d(input=x_input, filters=self.conv_filter[layer], strides=[1, 1, 1, 1], padding='SAME') + self.bias[layer])
    ...
    def calc_normality_test(self, residual_noise, batch_size, batch_size_for_norm_test):
        ...
        mean = tf.reduce_mean(input_tensor=residual_noise, axis=1)
        mean = tf.reshape(mean, [groups, 1])
        variance = tf.reduce_mean(input_tensor=tf.square(residual_noise - mean), axis=1)
        variance = tf.reshape(variance, [groups, 1])
        moment_3rd = tf.reduce_mean(input_tensor=tf.pow(residual_noise - mean, 3), axis=1)
        moment_3rd = tf.reshape(moment_3rd, [groups, 1])
        moment_4th = tf.reduce_mean(input_tensor=tf.pow(residual_noise - mean, 4), axis=1)
        ...
        norm_test = tf.reduce_mean(input_tensor=tf.square(skewness) + 0.25 * tf.square(kurtosis-3))
    ...
    def train_network(self, model_id):
        ...
        if self.train_config.normality_test_enabled:
            ...
            if self.train_config.normality_lambda != np.inf:
                training_loss = tf.reduce_mean(input_tensor=tf.square(y_out - y_label)) + training_loss * self.train_config.normality_lambda
                orig_loss_for_test = tf.reduce_mean(input_tensor=tf.square(y_label - x_in)) + orig_loss_for_test * self.train_config.normality_lambda
                test_loss = tf.reduce_mean(input_tensor=tf.square(y_label - y_out)) + test_loss * self.train_config.normality_lambda
        else:
            training_loss = tf.reduce_mean(input_tensor=tf.square(y_out - y_label))
            orig_loss_for_test = tf.reduce_mean(input_tensor=tf.square(y_label - x_in))
